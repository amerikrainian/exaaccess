...
#   tools\installer-e2e.ps1 [-Zip releases\ExaAccess-vX.Y.Z.zip] [-Cli installer\target\release\examples\cli.exe]
...
if (-not $Zip) { $Zip = Join-Path $root "releases\ExaAccess-v$version.zip" }
...
$game = Join-Path ([IO.Path]::GetTempPath()) ("exaaccess-e2e-" + [guid]::NewGuid().ToString("N"))
...
try {
    ...
    $env:EXAACCESS_INSTALLER_RELEASES_URL = "http://127.0.0.1:$Port/releases"
    ...
    Assert-True (-not (Test-Path (Join-Path $game "ExaAccess"))) "mod folder gone"
    ...
    Remove-Item Env:\EXAACCESS_INSTALLER_RELEASES_URL -ErrorAction SilentlyContinue
}
