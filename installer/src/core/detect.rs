...
pub fn game_candidates() -> Vec<GameInstall> {
    ...
    // EXAPUNKS_DIR is the same override the mod's own build uses for the game dir.
    if let Ok(value) = std::env::var("EXAPUNKS_DIR") {
        add_candidate(
            &mut result,
            &mut seen,
            PathBuf::from(value),
...
mod tests {
    ...
    fn write_markers(dir: &Path) {
        fs::write(dir.join("EXAPUNKS.exe"), "").unwrap();
        fs::write(dir.join(GAME_ASSEMBLY_MARKER), "").unwrap();
    }
    ...
    fn rejects_dir_without_game_assembly() {
        let dir = tempfile::tempdir().unwrap();
        fs::write(dir.path().join("EXAPUNKS.exe"), "").unwrap();
        assert!(!validate_game_dir(dir.path()));
        assert_eq!(
