...
mod tests {
    ...
    fn fresh_install_then_uninstall_leaves_game_dir_clean() {
        let dir = tempfile::tempdir().unwrap();
        fs::write(dir.path().join("EXAPUNKS.exe"), "game").unwrap();
    ...
    fn overwritten_file_is_backed_up_and_restored_on_uninstall() {
        let dir = tempfile::tempdir().unwrap();
        // A pre-existing EXAPUNKS.exe.config (say, a hand-copied install) must survive a
        // managed install + uninstall round trip.
        fs::write(dir.path().join("EXAPUNKS.exe.config"), "original loader").unwrap();

        let zip_path = dir.path().join("release.zip");
        create_zip(
            &zip_path,
            &[(paths::PLUGIN_REL, "plugin"), ("EXAPUNKS.exe.config", "new loader")],
        );
        ...
        let backup_rel = manifest
            .backups
            .get("EXAPUNKS.exe.config")
            .expect("existing file should be backed up")
            .clone();
        assert!(dir.path().join(&backup_rel).exists());
        assert_eq!(
            fs::read_to_string(dir.path().join("EXAPUNKS.exe.config")).unwrap(),
            "new loader"
        );
