...
namespace ExaAccess.Tests
{
    public class KeyGraphTests
    {
        ...
        public void ReconcileTier2FollowsStructuralKeyAcrossRebuilds()
        {
            ...
            var g = new KeyGraph(() =>
            {
                generation++; // fresh vtables/nodes each render â€” only the structural keys repeat
        ...
        {
            // Menu row, raw block (its own internal wiring), menu row â€” all one stop. Menu wiring must
        ...
        public void TreeOpsExpandCollapseDescendAscend()
        {
            ...
            var r = g.TreeRight(); // collapsed â†’ expand
            ...
            r = g.TreeRight(); // expanded â†’ descend to first child
            Assert.Equal(KeyGraph.TreeMove.Descended, r.Kind);
            Assert.Equal(Id("pause"), state.CurKey);

            r = g.TreeRight(); // a leaf inside the tree â€” consume
            Assert.Equal(KeyGraph.TreeMove.Leaf, r.Kind);

            r = g.TreeLeft(); // child â†’ ascend to the header
            Assert.Equal(KeyGraph.TreeMove.Ascended, r.Kind);
            Assert.Equal(Id("combat"), state.CurKey);

            r = g.TreeLeft(); // expanded header â†’ collapse
        ...
        public void FocusAndFocusByReferenceWork()
        {
            ...
            Assert.False(g.FocusByReference(backing)); // already there â€” not a change
