...
namespace Echopunks.Tests
{
    public class GraphBuilderTests
    {
        ...
        public void RaggedKeyedRowFallsToFirstItem()
        {
            ...
            // Column 2 doesn't exist below â†’ first item.
        ...
        public void GroupsEmitHeadersAndSuppressCollapsedSubtrees()
        {
            ...
            Assert.False(collapsed.Nodes.ContainsKey(Id("pause")));  // collapsed â†’ children swallowed
            ...
            Assert.True(expanded.Nodes.ContainsKey(Id("nested"))); // nested header visibleâ€¦
            Assert.False(expanded.Nodes.ContainsKey(Id("deep")));  // â€¦but its own subtree still collapsed
        ...
        public void PositionsAutoStampBySiblingGroup()
        {
            ...
            var render = new GraphBuilder(expansion)
                ...
                .EndGroup()
                .BeginStop()
                .AddItem(Id("lone"), Vt("Lone"))    // single sibling â†’ no position
                .BeginStop()
        ...
        {
            // A screen declaring list â†’ raw grid â†’ button must keep that Tab-stop order (raw nodes must
            // NOT be appended after all menu rows â€” that displaced FlowSheet stops behind later buttons).
            var render = new GraphBuilder()
        ...
        public void RawModeWiresExplicitEdges()
        {
            var render = new GraphBuilder()
                ...
                .Connect(Id("a"), GraphDir.Down, Id("ghost")) // undeclared â†’ dropped
                .SetStart(Id("b"))
                .Build();
