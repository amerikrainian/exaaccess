...
namespace ExaAccess.Input
{
    ...
    /// feels like the rest of the user's system (ported from WrathAccess). Queries user32
    /// SystemParametersInfo; falls back to sane defaults. Read once and cached (Refresh re-reads).
    /// </summary>
    public static class OsKeyboard
    {
        ...
        /// <summary>Force a re-read of the OS settings (e.g. the user changed them mid-session).</summary>
            ...
            try
            {
                    ...
                    SystemParametersInfo(SPI_GETKEYBOARDSPEED, 0, ref speed, 0))
                {
                    ...
                    Log.Info(string.Format(
                        "[input] OS key repeat: delay={0:0.###}s, interval={1:0.###}s (raw delay={2}, speed={3})",
                        _initialDelay, _repeatInterval, delay, speed));
                    return;
                }
                Log.Info("[input] OS key repeat query returned false; using defaults.");
            }
            catch (Exception e)
            {
                Log.Info("[input] OS key repeat unavailable, using defaults: " + e.Message);
            }
