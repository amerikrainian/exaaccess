...
namespace ExaAccess.UI.Graph
{
    ...
    /// value-equatable key â€” a string, or a composite such as a (region, row, col) tuple.</para>
    ///
    /// <para>Two controls are "the same" when their references are identical (tier 1 â€” a perfect match
    /// that follows an object that MOVED, its structural key changing) OR their structural keys are equal
    /// (tier 2 â€” follows a logical control whose backing object was rebuilt: new instance, same identity).</para>
    ///
    ...
    public sealed class ControlId : IEquatable<ControlId>
    {
        ...
        /// <summary>A control identified by a backing object only â€” the object doubles as the structural
