...
namespace ExaAccess.UI.Graph
{
    ...
    /// re-announces nothing but the child â€” the group is on the child's chain AND is the from-node, so the
    ...
    public static class GraphAnnouncer
    {
        ...
        public static string Compose(GraphNode from, GraphNode to, string transitionLabel = null)
        {
            ...
            // Common prefix by identity â€” levels we were already inside (or ON: descending from a group
            ...
            else
            {
                for (int j = i; j < toPath.Count; j++)
                {
                    ...
                    // itself â€” "a 'Game difficulty' section wrapping the 'Game difficulty' control").
                    if (j + 1 < toPath.Count)
                    {
        ...
        /// <summary>Pluggable per-part filter â€” installed by the host to consult the user's announcement
        ...
        /// with the node's own â€” a node part overrides a common part of the same kind â€” sorted by the
        ...
        public static List<NodeAnnouncement> EffectiveAnnouncements(GraphNode node)
        {
            ...
            {
                // Stable: composite key = (kind's order index, declaration index) â€” List.Sort alone is
        ...
        /// joined â€” plus, for an expandable group, its expanded/collapsed state word. The first part is
        ...
        /// <summary>The first announcement part's text (the label) â€” for dedupe and search fallbacks.</summary>
